/* Strip the Terminal's guide to the fields the public Atlas page renders.
   ...
   ships. Run after every refresh of public/ai-atlas/guide.json:
...
const path = new URL('../../public/ai-atlas/guide.json', import.meta.url);
...
const stripped = {
  ...
  steps: guide.steps.map(s => pick(s, ['id', 'title', 'location', 'text', 'children'])),
  entries: guide.entries.map(e => pick(e, ['id', 'title', 'text', 'detail', 'children'])),
  links: guide.links,
  system: {
    nodes: guide.system.nodes.map(n => pick(n, ['id', 'title', 'role', 'detail', 'basis'])),
...
console.log(`guide.json: ${before} -> ${after} bytes, ${guide.entries.length} entries, ${guide.system.nodes.length} nodes`);
