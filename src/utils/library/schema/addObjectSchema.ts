...
const coverImageSchema = z
  .instanceof(File)
  .refine((f) => COVER_MIME_TYPES.includes(f.type), 'Cover must be a JPEG, PNG or WebP image.')
  .refine((f) => f.size <= COVER_MAX_BYTES, 'Cover image must be 5 MB or smaller.');

const URL_REGEX = /^(https?:\/\/)?([\w-]+(\.[\w-]+)+)(\/[\w-./?%&=]*)?$/;

const bookSchema = z.object({
  type: z.literal('book'),
  title: z.string().min(1, 'Book title is required').max(200, 'Title must be 200 chars or less.'),
...
const videoSchema = z.object({
  ...
  sourceUrl: z.string().min(1, 'URL is required').regex(URL_REGEX, 'Enter a valid URL'),
  title: z.string().min(1, 'Video title is required').max(150, 'Title must be 150 chars or less.'),
  author: z.string().max(100, 'Creator must be 100 chars or less.').optional(),
  description: z.string().max(5000, 'Description must be 5000 chars or less.').optional(),
  coverImage: coverImageSchema.optional().nullable(),
});
...
const audioSchema = z.object({
  ...
  sourceUrl: z.string().min(1, 'URL is required').regex(URL_REGEX, 'Enter a valid URL'),
  title: z.string().min(1, 'Audio title is required').max(150, 'Title must be 150 chars or less.'),
