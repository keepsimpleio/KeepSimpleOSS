...
// baseline is the same ask arriving cold from a stranger (easy to refuse);
// the flagged variant is the identical ask after weeks of harmless,
// friendly pings. No quoted figures by policy (see project memory
// `feedback_offsec_no_mocked_numbers`) — familiarity-buys-trust is the
// pattern, not a fabricated conversion rate.

import type { OffsecBiasContent } from './types';

const content: OffsecBiasContent = {
  scenario:
    'Someone you’ve been chatting with for weeks finally asks for a small favour. Nothing about the ask changed — but your read on it did, because by now the name feels like a colleague, not a stranger. That warmth is the whole operation.',
  ...
  defense: {
    ...
    moves: [
      'Familiarity is not identity. Weeks of friendly chat don’t verify who’s on the other end — a known-feeling name still has to clear the same bar as a stranger before it gets a credential or a click.',
      'Judge the ask, not the rapport. Strip away the relationship and ask: would I sign in with my work account because a random account told me to? If no cold, then no warm either.',
