...
const content: OffsecBiasContent = {
  ...
  whyItWorks:
    'This is serial recall. We store processes as ordered sequences, and once steps arrive in the order memory expects, the mind stops judging each one on its own merits and accepts it because it landed in the right slot. The attacker copies the genuine verification exactly: name, then last transaction, then address. By the time step four arrives, it is accepted because the position matches, not because anyone weighed what it actually asks. Reading back a one-time code would ring alarms in isolation, but delivered as the next beat of a familiar sequence it reads as simply the next thing on the checklist. The order did the vouching, not the content. That is why the code read-back, the one step that hands over control, has to be judged alone, no matter how correctly the sequence ran up to it.',
