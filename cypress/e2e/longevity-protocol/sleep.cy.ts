const PAGE = '/tools/longevity-protocol/habits/sleep';

describe('Sleep – /tools/longevity-protocol/habits/sleep', () => {
  ...
  describe('Mobile (390x844)', () => {
    ...
    it('opens the sleep chart modal and closes it via the close icon', () => {
      // Scroll down so the button is reachable (it renders below the supplements section)
      ...
      // Modal is rendered immediately; html2canvas fills the image asynchronously
      cy.get('[data-cy="sleep-chart-modal"]').should('be.visible');

      // Wait for html2canvas to produce the data-URL – src changes from '' to data:image/png...
      cy.get('[data-cy="sleep-chart-img"]', { timeout: 15000 })
      ...
      cy.get('[data-cy="sleep-chart-modal"]').should('not.exist');
    });
  });
});
