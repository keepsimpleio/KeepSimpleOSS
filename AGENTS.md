...
| **Articles**                      | `/articles/<slug>`     | Strapi CMS                         | Long-form writing (~25 articles). Markdown mirrors at `public/llms-full-pages/article/*.md`.                                                                                                                  |
...
- **`public/llms.txt`** — index of every page with a one-line description (the `llms.txt` AI-discovery standard).
- **`public/llms-full.txt`** — full Markdown dump of all pages in one file.
- **`public/llms-full-pages/<section>/*.md`** — the same dump, split per page.
...
In production `next.config.js` serves `public/fonts` with a one-year immutable
`Cache-Control` and `public/assets` plus `public/library/images` with thirty
days. Replace an image under a **new file name** (suffix `-v2`, a hash, a
date); overwriting the old name leaves visitors on the cached copy for up to a
month. Development keeps the default headers so edits show on the DEV preview.

## Web fonts

CSS points at WOFF2 files; the TTF sources stay next to them because the
Library share thumbnail renders from TTF. `yarn fonts:build` regenerates every
WOFF2 (`scripts/fonts/build-webfonts.cjs`), including the unicode-range slices
for YujiSyuku and DelaGothicOne under `public/fonts/<Family>/` and their SCSS
partials in `src/styles/fonts/`. Never edit those partials by hand. When new
Japanese copy lands, refresh `scripts/fonts/yuji-syuku.hot.txt` as described in
`scripts/fonts/collect-hot-chars.md` and rebuild. `yarn fonts:check` verifies
the outputs match the sources.

## Generated Files

`public/llms-full-pages/` contains files generated by `scripts/generate-llms-pages.ts`. Do not hand-edit these. They are rebuilt by the `generate:llms:pages` script and committed via CI (`.github/workflows/generate-llms.yml`).
